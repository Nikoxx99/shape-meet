...
const sidecarPort = Number(process.env.SHAPE_AI_SMOKE_PORT ?? 0) || (await getFreePort());
...
const videoProcessor = await startJsonProcessor("video", (payload) => {
  ...
  assert(payload.identity?.id === "identity_smoke", "video processor did not receive identity manifest");
  assert(payload.enabled?.face === true, "video processor did not receive face=true");
  assert(payload.enabled?.background === true, "video processor did not receive background=true");
  assert(payload.background?.cleanPlate?.dataUrl?.startsWith("data:image/"), "video processor did not receive clean plate data URL");
  assert(payload.background?.cleanPlate?.width === 1280, "video processor did not receive clean plate width");
  assert(payload.frame?.frameDataUrl?.startsWith("data:image/"), "video processor did not receive a frame data URL");
...
const audioProcessor = await startJsonProcessor("audio", (payload) => {
  ...
  assert(payload.enabled?.voice === true, "audio processor did not receive voice=true");
  assert(payload.audio?.audioDataBase64, "audio processor did not receive audio data");
...
try {
  ...
  assert(health.data.mode === "adapter-contract", "sidecar did not start in adapter-contract mode");
  assert(health.data.diagnostics?.externalProcessors?.video === true, "video processor not reported as configured");
  assert(health.data.diagnostics?.externalProcessors?.audio === true, "audio processor not reported as configured");
  ...
  assert(frame.data.frame?.processor === "mock-facefusion-backgroundmattingv2", "frame did not come from video mock processor");
  ...
  assert(audio.data.audio?.processor === "mock-vcclient000", "audio did not come from audio mock processor");
  ...
  assert(status.data.session?.adapterError === null, `unexpected adapter error: ${status.data.session?.adapterError}`);
  assert(requests.video.length === 1, "video mock processor was not called exactly once");
  assert(requests.audio.length === 1, "audio mock processor was not called exactly once");

  const stopped = await request(`/sessions/${encodeURIComponent(sessionId)}`, { method: "DELETE" });
...
async function startJsonProcessor(name, handler) {
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      response.end(JSON.stringify({ error: `${name}_processor_failed`, message: error.message }));
    }
  });
  ...
  assert(address && typeof address === "object", `${name} processor did not bind a port`);
