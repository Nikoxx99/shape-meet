...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const input = updateRankSchema.parse(await request.json());
    ...
    if (targetUser.rank === "ADMIN") {
      return NextResponse.json({ error: "No se puede cambiar una cuenta admin desde este panel." }, { status: 403 });
    }
    ...
    return apiErrorResponse(error, { fallbackMessage: "No se pudo actualizar el rango del usuario." });
  }
}
