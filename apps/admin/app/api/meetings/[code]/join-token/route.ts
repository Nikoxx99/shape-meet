...
export async function POST(request: Request, context: { params: Promise<{ code: string }> }) {
  try {
    ...
    const isHost = Boolean(session && (session.user.id === meeting.hostId || session.user.rank === "ADMIN"));
    const participantCount = meeting.participants.filter((participant) => participant.joinedAt && !participant.leftAt).length;
    ...
    const pendingParticipant =
      !isHost && input.participantId
        ? meeting.participants.find((participant) => participant.id === input.participantId && participant.role !== "host")
        : null;

    if (!isHost && !pendingParticipant) {
    ...
    if (!isHost && !pendingParticipant?.admittedAt) {
      return NextResponse.json({ error: "El host aún no te ha admitido.", code: "WAITING_FOR_HOST" }, { status: 409 });
    }

    if (pendingParticipant?.leftAt) {
      return NextResponse.json({ error: "Esta solicitud ya no está activa.", code: "WAITING_ROOM_EXPIRED" }, { status: 409 });
    }
    ...
    return apiErrorResponse(error, { fallbackMessage: "No se pudo emitir acceso a la reunión." });
  }
}
